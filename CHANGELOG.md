...
- **新增「强制关闭代理」开关（默认开启）**：设置面板新增独立的「强制关闭代理」开关，默认开启；开启后无论「启用本地 OpenAI 兼容代理」如何设置，本地代理都不会启动、不占用端口。关闭该开关后，代理才按「启用本地 OpenAI 兼容代理」的配置运行。开关始终可见（不随陪伴插件直连隐藏），陪伴插件直连不受其影响。
