...
- **精准参考模式无图回退文生图**：`openai_reference_mode=director` 且请求未带参考图、也未配置兜底参考图时，原先直接报「🖼 该操作需要输入一张源图片」硬错误，导致陪伴插件联动生图整体失败；现按文档契约回退纯文生图并记录日志。仅 director-tools 图片处理动作（无源图无意义）保持强制要求源图。
