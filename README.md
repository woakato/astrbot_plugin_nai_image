...
> 🆕 **v2.2.0 联动新玩法**：陪伴插件「我会永远陪着你」已支持 **NAI联动模式**——由陪伴插件直接输出原生 NAI 标签 prompt，**无需再开启本插件的 LLM 转译**。详见下方「联动插件」一节。
...
`NAI_Generate_Image`：参数与 `/image` 命令相同，调用 NAI 生成一张图片并直接发送给当前用户，同时向 Agent 返回单一工具结果以保留完整对话历史。同一消息事件内的相同请求只会生成一次。
...
### 🆕 NAI联动模式（推荐，无需 LLM 转译）

陪伴插件在 **「长线主动 → 主动拍照/生图 → 生图提示词表达方式」** 中新增了 **「NAI联动模式」**：

- 开启后由陪伴插件的 LLM **直接生成原生 NAI 标签语法**的 prompt（`{tag}` 加权 / `[tag]` 降权、多角色包裹等 NAI 4.5 规范），经本代理**原样提交**生图
- 此模式下请**关闭本插件的 `enable_translate`**：本插件会跳过转译与服装池预处理、原样发送，只拼接「角色核心关键词与身体描述补全」，避免二次转译破坏原生语法
- 两种方案二选一：**陪伴插件开 NAI联动模式 + 本插件关转译**，或 **陪伴插件输出自然语言 + 本插件开转译**
