...
- 直连不依赖本地代理：`enable_proxy` 默认关闭，需要旧式代理时再手动开启（开启后仅影响下方"方式二"）。

### 方式二：本地 OpenAI 兼容代理（旧方式，仍可用）

插件启动后会在 `127.0.0.1:8765` 起一个 **OpenAI Images API 兼容** 的本地代理（`enable_proxy` 控制，**默认关闭**，需手动开启；端口可在配置里修改），让陪伴插件把生图请求转发到这里，由 nai.sta1n.cn 完成实际生图。注意：启用「陪伴插件直连」时设置面板会隐藏代理相关选项，先关闭直连才能看到 `enable_proxy` 开关。
