...
3. 本插件保持 `enable_companion_link` 开启（默认开启）。开启直连后，设置面板会自动隐藏「启用本地 OpenAI 兼容代理」「本地代理端口」「提示词转译模式」「转译模型供应商」——直连不走本地代理、提示词由陪伴侧按模式处理，这些选项不再需要；如需使用旧式代理，先关闭直连即可看到它们。
...
- 直连不依赖本地代理：设置面板里有独立的「强制关闭代理」开关（默认开启），开启时本地代理绝不启动、不占用端口；需要旧式代理时先关闭该开关（并确认「启用本地 OpenAI 兼容代理」为开启）。

### 方式二：本地 OpenAI 兼容代理（旧方式，仍可用）

插件启动后会在 `127.0.0.1:8765` 起一个 **OpenAI Images API 兼容** 的本地代理，让陪伴插件把生图请求转发到这里，由 nai.sta1n.cn 完成实际生图。代理是否真正启动由两个开关共同决定：**「强制关闭代理」（默认开启，优先级最高）** 与「启用本地 OpenAI 兼容代理」（默认开启）。默认情况下代理被强制关闭、不占端口；要用代理时，先关闭「强制关闭代理」即可。注意：启用「陪伴插件直连」时设置面板会隐藏「启用本地 OpenAI 兼容代理」与端口等选项（「强制关闭代理」始终可见），先关闭直连才能改它们。
