...
> 💡 陪伴插件在「长线主动 → 主动拍照/生图 → 生图提示词表达方式」中新增了 **「NAI联动模式」**：开启后由陪伴插件直接生成原生 NAI 标签语法的 prompt（`{tag}` 加权、多角色包裹等 NAI 4.5 规范），经本代理原样提交生图。此模式下建议关闭本插件的 `enable_translate`，避免二次转译破坏原生语法。
